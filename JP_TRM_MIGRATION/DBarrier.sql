...
define trades_file=DblBarrier.csv
define oas_file=DblBarrier_oa.csv
...
prompt *****************************************
prompt *   Double Barrier Trades Migration     *
prompt *****************************************
spool ^trades_file
...
SELECT 
    ...
    ||'^delimiter'|| to_char(NOE.TRADE_DATE,'^DATE_ONLY')
												-- D17	Trade Date
    ||'^delimiter'|| 'Double'					-- D18	Product Type
	...
	||'^delimiter'||NOE.LOWER_BARRIER_LEVEL		-- D43	Lower Barrier
	||'^delimiter'||''							-- D44	Calc Agent
	||'^delimiter'||NOE.REBATE_PERCENT_1		-- D45	LB Percent
	...
	||'^delimiter'||CASE 
					...
					END							-- D47	LB Payout
	||'^delimiter'||NOE.UPPER_BARRIER_LEVEL		-- D48	Upper Barrier		
	||'^delimiter'||NOE.REBATE_PERCENT_1		-- D49	UP Percent
	||'^delimiter'||DECODE(NOE.REBATE_QUOTE_MODE,1, ccy1.name,ccy2.name)
												-- D50	UB Rebate Ccy
	||'^delimiter'||CASE 
						WHEN NOE.REBATE_TYPE=0 then '' 
	...
	||'^delimiter'||'Orig Giveup Id:' || NOE.ID ||'|Orig Creation Time:' || 
					...
					END							-- D52	Notes
	||'^delimiter'||''							-- D53	ECN 
	||'^delimiter'||'2'							-- D54	CSV Version
	||'^delimiter'||''							-- D55	Is Anonymous	
	||'^delimiter'||reporting.string			-- D56	Origin System
...
WHERE NOE.option_type = 3 -- Double Barrier 
   -- Cut by date 
...
prompt *****************************************
prompt *   Double Barrier Allocations          *
prompt *****************************************
spool ^oas_file 
