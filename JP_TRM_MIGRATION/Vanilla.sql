...
define trades_file=vanilla.csv
define oas_file=vanila_oa.csv
...
prompt *********************************
prompt *   Vanilla Trades Migration    *
prompt *********************************
spool ^trades_file
...
SELECT 
    ...
    ||'^delimiter'|| to_char(NOE.TRADE_DATE,'^DATE_ONLY')
												-- D17	Trade Date
    ||'^delimiter'|| 'Vanilla'					-- D18	Product Type
	...
	||'^delimiter'||'Orig Giveup Id:' || NOE.ID ||'|Orig Creation Time:' || 
					...
					CASE 
						when (trade_type.string <> 'EXERCISE') then ''
    ...
    ||'^delimiter'||CASE when NOE.external_ref = 'TRAIANA_'||NOE.ID 
...
WHERE  NOE.option_type = 1 -- Vanilla
   -- Cut by date 
   ...
   AND trade_type.key = 'OptTradeType'
