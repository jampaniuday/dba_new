...
FROM   cas_prod.MSG_QUEUE_OUT QOUT
JOIN   cas_prod.MSG_QUEUE_IN QIN ON (QOUT.TXN_ID = QIN.TXN_ID)
JOIN   cas_prod.SD_ADDRESS ADDR ON (ADDR.ID = QIN.ADDRESS_ID)
...
FROM  ldata
JOIN  cas_prod.a_organization org ON (ldata.owner_id=org.id)
group by org.name;
spool off
exit
