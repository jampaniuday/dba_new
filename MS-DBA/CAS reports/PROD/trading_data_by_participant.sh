...
export OUTPUT_PATH=`date +"/cassrc/reports/%d-%m-%Y_PROD_trading_data_by_participant.csv"`
export OUTPUT_FILE_NAME=`date +"%d-%m-%Y_PROD_trading_data_by_participant.csv"`
cd /cassrc/reports
sqlplus read_only/limited @trading_data_by_participant.sql $OUTPUT_PATH > /dev/null
echo -e "Subject:CAS Service Management Reports (PROD) - Daily Trading Data by Participant\nFrom:Traiana Reports System <ms-dba@traiana.com>\n`uuencode $OUTPUT_PATH $OUTPUT_FILE_NAME`"| /usr/sbin/sendmail "vladimirb@traiana.com"
