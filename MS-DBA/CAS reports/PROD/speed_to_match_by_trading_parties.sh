...
export OUTPUT_PATH=`date +"/cassrc/reports/%d-%m-%Y_PROD_speed_to_match_by_trading_parties.csv"`
export OUTPUT_FILE_NAME=`date +"%d-%m-%Y_PROD_speed_to_match_by_trading_parties.csv"`
cd /cassrc/reports
sqlplus read_only/limited @speed_to_match_by_trading_parties.sql $OUTPUT_PATH > /dev/null
echo -e "Subject:CAS Service Management Reports (PROD) - Daily Speed to Match by Trading Parties\nFrom:Traiana Reports System <ms-dba@traiana.com>\n`uuencode $OUTPUT_PATH $OUTPUT_FILE_NAME`"| /usr/sbin/sendmail "vladimirb@traiana.com"
