...
create table cas_tickets as
select cas_prod.helper_pkg.GetOrganizationName(tp_id) Trading_party
        ,Regular
        ,Position
        ,Offset
        ,Aggregation
...
( select
    ...
    from cas_prod.fd_fxpb_linear_TA ta, cas_prod.fd_linear_leg_data leg, cas_prod.md_spot_rates rate
...
compute sum LABEL 'Total CAS PROD Blocks:' of "Total" "Regular" "Position" "Offset" "Aggregation" "Notional Total (USD)" "OFAC Scanned" "OFAC Hits" on report
break on report
spool ^1
select Trading_party "Trading party",Regular "Regular",Position "Position",Offset "Offset",Aggregation "Aggregation",Total "Total","Notional Total (USD)" "Notional Total (USD)","OFAC Scanned" "OFAC Scanned","OFAC Hits" "OFAC Hits" from cas_tickets;
spool off
exit;
