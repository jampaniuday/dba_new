...
SELECT TA.CREATION_DATE
       ...
       ||'^delimiter'||cas_prod.HELPER_PKG.GETMAPPEDCCYNAME(CCY_1)
       ||'^delimiter'||QTY_1
       ||'^delimiter'||cas_prod.HELPER_PKG.GETORGANIZATIONNAME(CP_ID)
       ||'^delimiter'||cas_prod.HELPER_PKG.GETMAPPEDCCYNAME(CCY_2)
       ||'^delimiter'||QTY_2
...
JOIN   cas_prod.FD_LINEAR_LEG_DATA LEG ON (TA.ID = LEG.PARENT_ID)
