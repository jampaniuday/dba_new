...
SELECT CASE WHEN NOE_LIFE_STATUS = 7 THEN 3 ELSE 1 END
       ...
       ||'^delimiter'||TP_ID
       ||'^delimiter'||'^quoteDelimiter'||CAS_PREPROD.HELPER_PKG.getOrganizationName(TP_ID)||'^quoteDelimiter'
       ||'^delimiter'||CP_ID
       ||'^delimiter'||'^quoteDelimiter'||CAS_PREPROD.HELPER_PKG.getOrganizationName(CP_ID)||'^quoteDelimiter'
           ||'^delimiter'||TO_CHAR(TRADE_DATE,'^dateFormat')
       ||'^delimiter'||TO_CHAR(VALUE_DATE,'^dateFormat')
           ||'^delimiter'||CCY_1
       ||'^delimiter'||'^quoteDelimiter'||CAS_PREPROD.HELPER_PKG.getMappedCCYName(CCY_1)||'^quoteDelimiter'
       ||'^delimiter'||QTY_1
       ||'^delimiter'||CCY_2
       ||'^delimiter'||'^quoteDelimiter'||CAS_PREPROD.HELPER_PKG.getMappedCCYName(CCY_2)||'^quoteDelimiter'
       ||'^delimiter'||QTY_2
...
FROM
    CAS_PREPROD.FD_FXPB_LINEAR_TA TA,
    CAS_PREPROD.FD_LINEAR_LEG_DATA LEG
WHERE TA.ID = LEG.PARENT_ID 
  AND ((
       ...
       (TA.TRADE_TYPE IN (11,10) AND LEG.NETTING_SERVICE_REF IN (
                                                                    ...
                                                                    from  CAS_PREPROD.FD_FXPB_LINEAR_TA TA1,CAS_PREPROD.FD_LINEAR_LEG_DATA LEG1
