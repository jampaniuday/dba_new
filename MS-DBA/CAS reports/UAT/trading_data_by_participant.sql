...
LEFT JOIN   cas_uat.FD_LINEAR_LEG_DATA LEG ON (TA.ID = LEG.PARENT_ID)
LEFT JOIN   cas_uat.SD_NETTING_GROUPS AGGR ON (LEG.NETTING_GROUP_ID = AGGR.ID)
LEFT JOIN   cas_uat.MD_INSTRUMENT_CCY_DATA CCYS1 ON (LEG.CCY_1 = CCYS1.ID) 
LEFT JOIN   cas_uat.MD_INSTRUMENT_CCY_DATA CCYS2 ON (LEG.CCY_2 = CCYS2.ID)
