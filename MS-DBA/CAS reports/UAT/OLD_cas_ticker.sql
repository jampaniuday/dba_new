drop table cas_uat_tickets;

create table cas_uat_tickets as
select cas_uat.helper_pkg.GetOrganizationName(tp_id) Trading_party
        ,Regular
        ,Position
        ,Offset
        ,Aggregation
...
( select
    ...
    from cas_uat.fd_fxpb_linear_TA ta, cas_uat.fd_linear_leg_data leg, cas_uat.md_spot_rates rate
...
col "Trading party" format a21
compute sum LABEL 'Total CAS UAT Blocks:' of "Total" "Regular" "Position" "Offset" "Aggregation" "Notional Total (USD)" "OFAC Scanned" "OFAC Hits" on report
break on report
spool ^1
select Trading_party "Trading party",Regular "Regular",Position "Position",Offset "Offset",Aggregation "Aggregation",Total "Total","Notional Total (USD)" "Notional Total (USD)","OFAC Scanned" "OFAC Scanned","OFAC Hits" "OFAC Hits" from cas_uat_tickets;
spool off
exit;
