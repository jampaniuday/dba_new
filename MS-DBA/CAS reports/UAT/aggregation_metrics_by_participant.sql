...
from cas_uat.fsd_netting_executions Aggs
LEFT JOIN cas_uat.fsd_netting_fx_results Aggs_R ON (Aggs.net_execution_id = Aggs_R.net_id)
LEFT JOIN cas_uat.FD_LINEAR_LEG_DATA LEG ON (Aggs_R.id = LEG.netting_service_ref)
LEFT JOIN cas_uat.FD_FXPB_LINEAR_TA TA ON (LEG.PARENT_ID = TA.ID)
LEFT JOIN cas_uat.MSG_QUEUE_OUT QOUT ON (to_char(TA.id) = QOUT.msg_ref)
where 
