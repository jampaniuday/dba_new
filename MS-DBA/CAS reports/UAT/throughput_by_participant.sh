...
export OUTPUT_PATH=`date +"/cassrc/reports/uat/%d-%m-%Y_UAT_throughput_by_participant.csv"`
export OUTPUT_FILE_NAME=`date +"%d-%m-%Y_UAT_throughput_by_participant.csv"`
cd /cassrc/reports/uat
sqlplus read_only/limited @throughput_by_participant.sql $OUTPUT_PATH > /dev/null
echo -e "Subject:CAS Service Management Reports (UAT) - Daily Throughput by Participant\nFrom:Traiana Reports System <ms-dba@traiana.com>\n`uuencode $OUTPUT_PATH $OUTPUT_FILE_NAME`"| /usr/sbin/sendmail "vladimirb@traiana.com"
