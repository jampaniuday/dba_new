...
/home/oracle/scripts/rman_full_backup_parallel.sh "sys/$SYS_PASS@$BACK_SID" "$CATALOG_USER/$CATALOG_PASS@$CATALOG" $BACK_SID $BACKUP_DIR $ORACLE_HOME  > ${CENTRAL_LOG_DIR}/RMAN_${DBNAME}_Full.$START_TIME.log

#######################################################################
# check Backup status
#######################################################################

CURRENT_LOG_FILE=`ls -ltr ~/scripts/logs/RMAN_Main_Backup_*_$BACK_SID |tail -1 | awk '{print $9}'`
...
echo "find $CENTRAL_LOG_DIR -follow -name *.log* -mtime +${LOG_DAYS_TO_KEEP} |xargs rm -f"
find "$CENTRAL_LOG_DIR" -follow -name "*.log*" -mtime +${LOG_DAYS_TO_KEEP} |xargs rm -f
