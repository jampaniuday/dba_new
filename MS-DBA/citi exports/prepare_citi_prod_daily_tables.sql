...
create table A_TA as select * from A_TA@citi_prod where CREATED_DATE > current_date-1;

create table A_OA as select * from A_OA@citi_prod where CREATION_TIME > current_date-1;

create index idx_a_ta_tmp on a_ta(id);

create table M_FX_NOE as select * from M_FX_NOE@citi_prod where ID in (select id from A_TA);

create table A_TA_TA_REL as select * from A_TA_TA_REL@citi_prod where PARENT_TA_ID in (select id from A_TA) or CHILD_TA_ID in (select id from A_TA);

create table M_FX_OPTION_NOE as select * from M_FX_OPTION_NOE@citi_prod where ID in (select id from A_TA);
