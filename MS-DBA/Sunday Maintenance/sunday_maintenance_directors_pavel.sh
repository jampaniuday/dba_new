...
export SEND_TO=razi@traiana.com,alonp@traiana.com,galc@traiana.com
export RC=0

cd /oracle/sundaymaint
...
REPORT_NAME_FULL_PATH=$BASEDIR/`cat $LAST_RUN_FILE`
export ATTACH_NAME=$REPORT_NAME.html
export ATTACH_FILE=`echo $ATTACH_NAME`
export DB_LIST="'PAVEL'"

sqlplus sm/sm @/oracle/sundaymaint/dumphtml_per_db.sql $REPORT_NAME "$DB_LIST" $REPORT_NAME_FULL_PATH

mv $REPORT_NAME_FULL_PATH.lst $REPORT_NAME_FULL_PATH.html

$SCRIPT_HOME/sendattachment.sh $ATTACH_FILE $SEND_TO

mv $ATTACH_FILE PAVEL_$ATTACH_FILE
mv PAVEL_$ATTACH_FILE reports
