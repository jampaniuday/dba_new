...
do
  ...
  CURRENT_LOG_FILE=`ssh -f -i $SCRIPT_HOME/grid.key $HOST_ADDR "$COMMAND" | awk '{print $9}'`

  echo "Current Log File=>$CURRENT_LOG_FILE"

  EXPORT_ERRORS=`ssh -f -i $SCRIPT_HOME/grid.key $HOST_ADDR "cat $CURRENT_LOG_FILE | grep ORA-"`
